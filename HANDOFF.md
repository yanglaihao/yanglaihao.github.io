...
更新时间：2026-08-11 10:25 CST

本轮进一步精简杨来浩在 `团队成员 > 教职工 > 教师` 中的个人简介，并按用户要求直接部署：

- 删除简介卡片中的“团队领导”标签，保留姓名、照片及原有成员区排版。
- 简介最终收敛为一段两句式文字：第一句仅保留西安交通大学机械工程学院副研究员、博士身份；第二句列出 `IEEE Sensors Reviews Associate Editor`、`Cyborg and Bionic Systems（Science Partner Journal）Young Editor` 和中国自动化学会机器人专业委员会委员三项代表性任职。
...
- 团队成员改为两级点击结构：一级为 `教职工 / 在校生 / 已毕业`；二级为教职工下的 `教师 / 博士后`，在校生和已毕业下的 `博士研究生 / 硕士研究生 / 本科生`。
- 原“团队领导”与“教师 / 合作导师”合并到 `教职工 > 教师`，保留杨来浩与孙瑜现有信息。
- 杨浙帅、金若尘移入 `已毕业 > 博士研究生`，毕业去向分别为乾元实验室、西安热工院。
- 薛晨菲、姚晨彧、谢时雨、赵子攀移入 `已毕业 > 硕士研究生`；吕冠桥移入 `已毕业 > 本科生`。仅保留已有年级、学校与研究方向，未虚构毕业年份或去向。
- 中英文分类、姓名、去向与新增文本均已同步。
- 近期亮点工作图片无法显示的原因是欢迎页动态数据只读取不存在的 `data-figure`；现改为优先读取 `data-figure`，缺失时自动使用亮点成果视频的 `poster`，并保留普通图片 `src` 兜底。
- 本地验证通过：`node --check script.js`、`node tests/site-checks.js`；Chrome 桌面端与移动端实测确认两级切换、7 人毕业分类、英文翻译、无横向溢出以及 3 张不同亮点图均正常。

本轮根据用户提供的 2026 年中国自动化学会机器人专委会年会会议通知和用户确认信息，新增委员任职记录：

- 团队动态新增 2026-07-31 新闻 `杨来浩入选中国自动化学会机器人专业委员会委员`，按时间倒序置于新闻列表首位。
- `社会服务 > 学会委员` 新增 2026 年 `中国自动化学会机器人专业委员会委员`，置于社会服务列表首位。
- 英文标题与任职名称同步补齐；未增加链接、任期、职务层级或其他未确认信息。
- 团队动态当前为 3 条亮点报道、24 条新闻、4 条通知；社会服务当前共 16 条。
- 本地验证通过：`node --check script.js`、`node tests/site-checks.js`；Chrome 桌面端与移动端实测确认中文新闻首项、学会委员筛选、英文翻译和页面横向溢出检查均正常。
- 已部署至 GitHub Pages；功能更新提交为 `50706c9a604b6cbbef3677fa69e2156652ef0a1c`，Pages 构建成功，线上 HTML 与英文翻译均已核验。

本轮根据西安交通大学新闻网报道及苏梦颖博士当日讲座照片新增团队札记：

- 新增 2026-07-13 研究札记 `《Nature Mechanical Engineering》主编苏梦颖博士讲座札记`，并设为团队札记默认打开项；英文标题为 `Lecture Notes from Dr. Mengying Su, Chief Editor of Nature Mechanical Engineering`。
...
- 札记标题最终调整为 `《Nature Mechanical Engineering》主编苏梦颖博士讲座札记`，英文同步为 `Lecture Notes from Dr. Mengying Su, Chief Editor of Nature Mechanical Engineering`，并部署至 GitHub Pages。
...
- 已完成 GitHub Pages 线上部署；线上脚本确认注释性副标题已移除，讲座图片返回 HTTP 200，Pages 构建成功。

本轮按用户要求调整社会服务栏目位置与结构，并同步中英文内容和回归检查：

- 已将原团队成果中的“兼职 / 社会任职”类内容从团队成果栏目剥离，新增独立栏目 `社会服务`。
- 栏目位置已调整为 `团队成果` 与 `团队札记` 之间，导航顺序同步为：欢迎页、研究方向、团队动态、团队成员、团队成果、社会服务、团队札记、联系方式。
- `社会服务` 下设 4 个二级分类：`期刊编委`、`学会委员`、`会议主席`、`社会兼职`；`江苏省科技副总` 归入 `社会兼职 · 科技服务`。
- 根据 IROS 2025 PaperCept 页面补充 `IROS 2025 Soft Robot Materials and Design 3 分会 Co-Chair`，并按用户确认将 session organizer 也纳入会议主席类。
- 根据 2024 SES Annual Technical Meeting 页面补充 `Track 5.5 Mini-Invasive Robotic Manipulation 专题组织者`，组织者信息使用 `Yu Sun、申亚京、Laihao Yang`。
- 根据 China Daily Hong Kong、China Daily 昆明频道、IROS 2025、2024 SES 页面新增团队动态，并同步英文网页文本。
- 全站成果条目已按年份由新到旧重新排序；团队成果当前保留 `亮点成果、项目、论文、专利、专著、获奖`，社会服务独立排序。
...
- 根据本地 PDF `会议手册-ICAM 2026（终）.pdf` 新增 2026-07-04 新闻 `杨来浩受邀参加 ICAM 2026 并作主讲报告`。
- PDF 显示：杨来浩副研究员受邀参加第三届航空航天与力学国际学术会议（ICAM 2026），并在“航空航天先进材料结构智能制造、运维与可靠性”分论坛作主讲报告 `机器人具身智能赋能装备智能运维：进展与挑战`。
- 根据 `/Users/howie/Downloads/检索结果-大为检索.html` 补充中国专利 `CN122378663A`：`一种用于管阵列巡检的五自由度爬杆机器人及方法`，申请日 2026-04-15，公开日 2026-07-14，权利人为西安交通大学。
- 大为检索保存页当前离线可读为第一页 20 条；与主页现有专利号比对后，19 条已收录或已有授权版，新增缺失的 `CN122378663A`，不重复加入 `CN120134362A`、`CN119334645A`、`CN119321890A` 等已有授权版对应的公开记录。
- `script.js` 同步新增 ICAM 2026 新闻、`CN122378663A` 专利标题和结构化元信息的英文翻译。
- `tests/site-checks.js` 将新闻数量更新为 19、中国专利数量更新为 97、专利公开状态数更新为 47，并新增 ICAM 2026 新闻与大为专利回归检查。

上一轮按用户提供的西安交大新闻网链接和本地 Word 新闻稿新增团队动态：

- 新增 2026-07-13 新闻 `《自然-机械工程》主编苏梦颖博士访问西安交通大学`，标题链接至 `https://news.xjtu.edu.cn/info/1219/233773.htm`。
- 本地文件 `苏梦颖主编访问西安交通大学新闻稿（修订版）.docx` 显示：7月13日，应西安交通大学机械工程学院杨来浩副研究员邀请，《自然-机械工程》（Nature Mechanical Engineering）主编苏梦颖博士访问西安交通大学，并围绕工程出版、前沿研究与国际学术传播开展专题报告、座谈交流和实验室参访。
- `script.js` 同步新增该新闻标题和正文的英文翻译。
- `tests/site-checks.js` 将新闻数量更新为 18，并新增标题、正文、原文链接和英文翻译回归检查。
- 注：当前环境直连 `news.xjtu.edu.cn` 返回连接拒绝 / TLS 中断，正文依据用户提供链接对应的本地 Word 新闻稿修正。
...
上一轮按用户要求继续根据本地保存的知网检索页补齐主页中缺失的以往论文成果：

- 重新解析 `高级检索-中国知网.html` 中 16 条论文记录，并与当前主页 95 条论文成果按标题去重。
- 已有记录不重复添加；其中 2025 年航空学报记录由上一轮的英文 Scholar 形态改为知网中文题名 `连续压缩感知叶端定时频率估计方法`，分类保持 `paper-ei-journal`，链接改为知网记录。
- 新增 9 条此前主页未列出的知网论文记录：
  - `工程专业认证驱动下的思政元素挖掘与教学实践——以“工程有限元与数值计算”课程为例`，分类为 `paper-other`，来源 `教育教学论坛`。
  - `单传感叶端定时信号固有频率动态提取方法`，分类为 `paper-other`，来源 `计测技术`。
  - `新工科视域下“工程有限元与数值计算”双语课程体系探索与实践`，分类为 `paper-other`，来源 `教育教学论坛`。
  - `介电液体增强的静电驱动仿生章鱼吸盘`，分类为 `paper-other`，来源 `2024年中国航天大会论文集`。
  - `裂纹叶盘转子系统振动建模、机理分析与监测诊断`，分类为 `paper-other`，来源 `第15届全国转子动力学学术大会摘要集`。
  - `高模态密度结构宽频振动分析的小波有限元方法实现`，分类为 `paper-ei-journal`，来源 `振动与冲击`。
  - `叶端定时中多重信号分类法的滤波特性研究`，分类为 `paper-ei-journal`，来源 `西安交通大学学报`。
  - `齿轮磨损故障动态响应特征与诊断指标研究`，分类为 `paper-ei-journal`，来源 `机械工程学报`。
  - `增强稀疏分解及其在叶片振动参数识别中的应用`，分类为 `paper-ei-journal`，来源 `机械工程学报`。
- `script.js` 同步新增上述中文题名、期刊/会议名称和作者姓名的英文翻译，保证英文状态下成果页不会丢失关键信息。
- `tests/site-checks.js` 将论文成果数量更新为 104，将 `paper-other` 数量更新为 7，并新增知网历史记录存在性与分类回归检查。
- `README.md` 同步将代表性论文数量更新为 104。

上一轮按用户要求根据本地保存的知网与 Google Scholar 页面补齐论文成果条目：
...
- 已撤回本轮 Product Design 本地预览样式：
  ...
  - `index.html` 中样式表链接恢复为 `styles.css`，不再使用预览缓存版本号。
- 已将团队动态中的“杨来浩受聘担任 Cyborg and Bionic Systems Young Editor”同步增加到团队成果的“社会任职”栏目：
  - 新增 2026 年 `data-output-type="service"` 成果条目，分类为“社会任职 · 青年编委”，标题为 `Cyborg and Bionic Systems Young Editor`。
  - 条目说明为“任期 2026.07.01-2028.06.30 · Science Partner Journal”。
...
- 本地新增专利成果整理：成果区“专利”下保留“国际专利 / 中国专利”二级标题与筛选；按保存的 Google Patents 页面补齐国际专利 8 条；按四个保存的中国知网页面与本轮大为检索结果去重整理中国专利 97 条，其中 52 条发明授权、45 条发明公开。同一专利若已授权，仅保留授权号与授权信息。
...
团队成员区采用两级点击切换结构，默认展开 `教职工 > 教师`：

- 一级：`教职工 / 在校生 / 已毕业`
- 教职工二级：`教师 / 博士后`
- 在校生、已毕业二级：`博士研究生 / 硕士研究生 / 本科生`

教师分组包含杨来浩副研究员与孙瑜副教授，不再设置单独的“团队领导”分组或标签。孙瑜老师公开页面中的成员已按培养阶段并入既有分类；杨浙帅、金若尘、薛晨菲、姚晨彧、谢时雨、赵子攀和吕冠桥已移入对应的已毕业分类。博士后部分仍为描述性占位，未列出更多姓名。
